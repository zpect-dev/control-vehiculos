...
export function SelectField({ id, label, value, options = [], onChange }: SelectFieldProps) {
    ...
    // 1. Encontrar la opción seleccionada actual
    // Manejamos el caso donde value sea null o un string/number
    const selectedOption = useMemo(
        () => options.find((opt) => String(opt.value) === String(value)) || null,
        [options, value]
    );

    // 2. Filtrar las opciones según lo que el usuario escriba
    ...
    return (
        <div className="flex flex-col gap-1">
            ...
            {/* Nota: Eliminé la etiqueta <p> duplicada invisible que tenías en tu código original para limpiar el DOM */}

            <Combobox
                value={selectedOption}
                onChange={(option) => {
                    // Si se selecciona algo, enviamos el valor. Si se limpia, enviamos un string vacío.
                    onChange(id, option ? option.value : '');
                }}
                onClose={() => setQuery('')} // Limpia la búsqueda al cerrar el dropdown
            >
